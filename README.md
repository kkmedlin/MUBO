...
MUBO is a bilevel optimization-powered sampling approach developed to mitigate model bias caused by imbalanced training data. Here, bilevel optimization provides a framework for not one, but two optimization steps -- one for classic ML learning that finds optimal model parameters and the second for finding optimal training data. 
